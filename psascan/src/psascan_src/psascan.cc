...
void psascan_private::pSAscan(std::string input_filename, std::string output_filename,
                    std::string gap_filename, long ram_use,
                    long max_threads,
                    bool verbose, long gap_buf_size) {
    ...
    if (ram_use_excluding_threads < 6L) {
        ...
        fprintf(stderr,
                "Error: not enough memory to start threads. You need "
                "at least %ldMiB\n", required_MiB + 1);
        std::exit(EXIT_FAILURE);
    }
    ...
    fprintf(stderr, "RAM budget (excluding threads) = %ld (%.1LfMiB)\n",
            ...
            1.L * ram_use_excluding_threads / (1L << 20));
    long max_block_size = std::max(2L, (long) (ram_use_excluding_threads /
                                               5.2L));

    fprintf(stderr, "Max block size = %ld (%.1LfMiB)\n\n", max_block_size,
            1.L * max_block_size / (1L << 20));
    ...
    if (max_block_size < (1L << 31)) {
        std::vector<half_block_info<int> > hblock_info = partial_sufsort<int>(
                input_filename,
                output_filename, gap_filename, length, max_block_size,
                ram_use, max_threads, gap_buf_size, verbose);
        merge<int>(output_filename, ram_use, hblock_info);
    } else {
        std::vector<half_block_info<uint40> > hblock_info = partial_sufsort<uint40>(
                input_filename,
                output_filename, gap_filename, length, max_block_size,
                ram_use, max_threads, gap_buf_size, verbose);

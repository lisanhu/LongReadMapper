...
int seq_lookup(const mta_entry *table, int len, uint64_t loc, uint32_t qlen, seq_meta *result) {
#pragma acc loop seq
    for (int i = 0; i < len; ++i) {
...
static inline int single_end(int argc, const char *argv[]) {
    ...
    init(&ctx, argc, argv);
    timer = log.mvlog(&log, "Done initializing, begin loading reference file %s", ctx.genome);
    ...
    while ((len = reads_load(reads, ctx.batch_size, seq)) > 0) {
        ...
        log.mvlog(&log, "Done loading %ld queries in %lfs", len, time_elapse(timer));
            ...
            for (u64 i = 0; i < len; ++i) {
                ...
                for (int iter = 0; iter < sl + gl; ++iter) {
                    ...
                    for (int j = iter; j < r.len - sl; j += sl + gl) {
                        ...
                        rr = lc_aln(r.seq.s + j, ctx.seed_len, &kk, &ll, ctx.fmi, ctx.lch);
                ...
                char *cigar = cigar_align(r.seq.s, r.len, ctx.content + m.loc, r.len, &limit);
        ...
        for (int i = 0; i < len; ++i) {
            if (results[i].valid) {
                ...
                fprintf(out_stream,
                        ...
                        "%.*s\t"     // quality string
                        "ED:I:%d\n", // comment
                        (int)results[i].q_name.l, results[i].q_name.s, results[i].flag, (int)results[i].g_name.l,
                        results[i].g_name.s, results[i].off + 1, results[i].mapq, (int)results[i].CIGAR.l,
                        results[i].CIGAR.s, (int)results[i].r_name.l, results[i].r_name.s, results[i].r_off, 0,
                        (int)results[i].query.l, results[i].query.s, (int)results[i].qual.l, results[i].qual.s,
                        results[i].ed);
            }
